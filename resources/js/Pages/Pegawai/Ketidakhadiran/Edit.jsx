...
export default function KetidakhadiranEdit({ ketidakhadiran }) {
    const { data, setData, post, processing, errors } = useForm({
        _method: 'PUT',
        keterangan: ketidakhadiran.keterangan || 'Izin',
        tanggal: typeof ketidakhadiran.tanggal === 'string' ? ketidakhadiran.tanggal.slice(0, 10) : '',
        deskripsi: ketidakhadiran.deskripsi || '',
        file: null,
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        post(route('pegawai.ketidakhadiran.update', ketidakhadiran.id));
    };

    return (
        <PegawaiLayout>
            <Head title="Edit Pengajuan Ketidakhadiran" />
            ...
            <div className="max-w-2xl mx-auto space-y-6">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-extrabold text-slate-900 tracking-tight">Edit Pengajuan Izin</h1>
                        <p className="text-sm text-slate-500 mt-0.5">Perbarui rincian atau bukti pengajuan izin kamu.</p>
                    </div>
                </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5">
                        ...
                        <div>
                            <label className="block text-xs font-bold text-slate-600 uppercase mb-1.5">
                                Ganti Bukti Surat / Dokumen Pendukung (Opsional)
                            </label>
                            {ketidakhadiran.file && (
                            ...
                            <div className="rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50/60 p-5 text-center">
                                ...
                                <label htmlFor="file-upload" className="cursor-pointer inline-flex flex-col items-center">
                                    ...
                                    <span className="text-xs font-bold text-blue-600 hover:underline">
                                        {data.file ? data.file.name : 'Upload file baru jika ingin mengganti'}
                                    </span>
                        ...
                        <div className="pt-4 flex items-center justify-end gap-3 border-t border-slate-100">
                            ...
                            >
                                {processing ? 'Menyimpan...' : 'Perbarui Pengajuan'}
                            </button>
                        </div>
                    </form>
                </div>
